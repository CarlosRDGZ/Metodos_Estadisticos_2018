...
// John D. Cook Consultig

class AOVRandomizeBlocks extends AOV {
    constructor(data, treatments, blocks) {
        super(data, treatments, blocks);
    }

    get yDotJ() {
        let sumsByBlock = [];
        for(var j = 0; j < this._blocks; j++) {
            let sum = 0;
            for(var i = 0; i < this._treatments; i++)
                sum += this._data[i][j];
            sumsByBlock.push(sum);
        }
        return sumsByBlock;
    }

    get yDotJMean() {
        return this.yDotJ.map(e => e / this._treatments)
    }

    get deegresOfFreedomError() {
        return super.deegresOfFreedomError - this._blocks + 1;
    }

    get deegresOfFreedomBlock() {
        return this._blocks - 1;
    }

    get sumOfSquaresBlock() {
        return (this.yDotJ.map(e => e * e).reduce((a,b) => a + b, 0) / this._treatments) -
            (this.yDotDot * this.yDotDot) / (this._treatments * this._blocks);
    }

    get sumOfSquaresError() {
        return super.sumOfSquaresError - this.sumOfSquaresBlock;
    }

    get meanSquareBlock() {
        return this.sumOfSquaresBlock / this.deegresOfFreedomBlock;
    }

    get _SyB() {
        return Math.sqrt(this.meanSquareError / this._treatments);
    }

    get qB() {
        let pop =
            this._blocks - 2; // Number of populations
     ...
     */
    get hSDB() {
        return this.qB * this._SyB;
    }

    get fCalB() {
        return this.meanSquareBlock / this.meanSquareError;
    }
    ...
    get fTablasB() {
        ...
        if (this.deegresOfFreedomBlock <= 10)
            dFN = this.deegresOfFreedomBlock - 1;
        else if (this.deegresOfFreedomBlock == 12)
            dFN = 10;
        else if (this.deegresOfFreedomBlock == 15)
            dFN = 11;
        else if (this.deegresOfFreedomBlock == 20)
            dFN = 12;
        else if (this.deegresOfFreedomBlock == 24)
            dFN = 13;
        else if (this.deegresOfFreedomBlock == 30)
            dFN = 14;
        else if (this.deegresOfFreedomBlock == 40)
            dFN = 15;
        else if (this.deegresOfFreedomBlock == 60)
            dFN = 16;
        else if (this.deegresOfFreedomBlock == 120)
            dFN = 17;
        else
            dFN = 18;
...
module.exports = AOVRandomizeBlocks;
